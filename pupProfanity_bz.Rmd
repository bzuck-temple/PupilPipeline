...
timebinsmm <- baseline_pupil  %>% 
  mutate(pupilmm = (sqrt(movingavgpuup) * (0.119789246)))
...
```{r }
plotBySubj <- ggplot(agg_subject, aes(timebins, aggbaseline, color = subject)) +
  geom_smooth(method = loess) + facet_grid(.~ cond)

plotByCond <- ggplot(agg_cond, aes(timebins, aggbaseline)) +
  geom_smooth(method = loess) + facet_grid(.~ cond)
